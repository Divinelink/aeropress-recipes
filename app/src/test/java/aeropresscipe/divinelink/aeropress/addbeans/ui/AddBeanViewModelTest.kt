...
class AddBeanViewModelTest {
  ...
  @Test
  fun `given Initial state when updating any bean parameter then I expect InsertBean state`() =
    runTest {
      testRobot
        .buildViewModel()
        .onBeanNameChanged("name changed")
  ...
  @Test
  fun `given a selected option for RoastLevel, when roastLevelClicked, then I expect BottomSheet content with selected option`() =
    runTest {
      val roastLevelContent = RoastLevel.values().map { roastLevel ->
        UIText.StringText(roastLevel.name)
      }.toMutableList()
      ...
      testRobot
        ...
        .assertViewState(
          AddBeanViewState.ModifyBean(
            ...
            bottomSheetContent = processMethodContent,
            bottomSheetTitle = UIText.ResourceText(R.string.AddBeans__select_process_method),
            bottomSheetSelectedOption = UIText.ResourceText(
              ProcessMethod.CarbonicMaceration.stringRes,
            ),
          ),
        )
    }
