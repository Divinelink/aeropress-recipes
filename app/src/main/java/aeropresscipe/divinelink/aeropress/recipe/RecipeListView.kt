...
import com.divinelink.aeropress.recipes.databinding.ListRecipeItemBinding
...
class RecipeListView(
  ...
  override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
    ...
    val stepText = when (val step = steps[position]) {
      is RecipeStep.HeatWaterStep -> context.getString(
        step.stepText,
        step.waterAmount,
        step.temperature,
        step.temperature.toFahrenheit(),
      )
      is RecipeStep.CoffeeGrindStep -> context.getString(
        step.stepText,
        step.coffeeAmount,
        step.grindSize.size,
      )
      ...
      is RecipeStep.WaitToBrewStep -> context.getString(
        ...
        context.resources.getQuantityString(
          R.plurals.minutes,
          step.minutes.toInt() + step.seconds.toInt(),
        ),
      )
