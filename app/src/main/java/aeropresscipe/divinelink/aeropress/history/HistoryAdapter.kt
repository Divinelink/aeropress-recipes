...
class HistoryAdapter(
  ...
  override fun register() {
    ...
    registerFactory<EmptyType.EmptyHistory>(
      LayoutFactory(
        { layoutInflater, root ->
                EmptyViewHolder(
                  EmptyRecyclerLayoutBinding.inflate(
                    layoutInflater,
                    root,
                    false,
                    ),
  ...
  class HistoryMappingViewHolder(
    ...
    private fun updateWithAnimation(like: Boolean) {
      when (like) {
        true -> binding.card.binding.likeButton.setMinAndMaxFrame(
          SaveRecipeCardView.LIKE_MIN_FRAME,
          SaveRecipeCardView.LIKE_MAX_FRAME,
        )
        false -> binding.card.binding.likeButton.setMinAndMaxFrame(
          SaveRecipeCardView.DISLIKE_MIN_FRAME,
          SaveRecipeCardView.DISLIKE_MAX_FRAME,
        )
      }
