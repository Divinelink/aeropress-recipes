...
class SetTimerSoundUseCaseTest {
  ...
  @Test
  fun `given timer sound is false, when I update timer, then I expect timer sound to be true`() =
    runTest {
  ...
  @Test
  fun `given timer sound is true, when I update timer, then I expect timer sound to be false`() =
    runTest {
      val response = Result.Success(Unit)
      val failResponse = Result.Error(Exception("Some exception"))
