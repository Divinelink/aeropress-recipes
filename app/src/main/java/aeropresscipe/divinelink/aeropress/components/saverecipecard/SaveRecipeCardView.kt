...
class SaveRecipeCardView :
  ...
  var binding: ViewSaveRecipeCardBinding =
    ViewSaveRecipeCardBinding.inflate(LayoutInflater.from(context), this, false)
  ...
  override fun onAttachedToWindow() {
    ...
    val viewModelFactory =
      SaveRecipeCardViewModelFactory(assistedFactory, WeakReference<ISaveRecipeCardViewModel>(this))
    viewModel = ViewModelProvider(
      findViewTreeViewModelStoreOwner()!!,
      viewModelFactory,
    )[SaveRecipeCardViewModel::class.java]
  ...
  override fun handleShowSnackBar(state: SaveRecipeCardState.ShowSnackBar) {
    Notification
      .make(
        binding.saveRecipeButton,
        resources.getString(state.value.string, context.getString(state.value.favorites)),
      )
      .show()
  }
