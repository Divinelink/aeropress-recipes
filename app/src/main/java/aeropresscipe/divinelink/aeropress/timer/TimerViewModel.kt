...
class TimerViewModel @Inject constructor(
  ...
  override fun updateTimer() {
    ...
    if (brew?.getCurrentState() == BrewState.Finished) {
      ...
      startTimerStates(
        brew?.getCurrentState(),
        brew?.getCurrentState()?.brewTime.inMilliseconds(),
        true,
      )
    }
...
sealed class TimerState {
  ...
  data class UpdateDescriptionState(val brewState: BrewState, val animateDescription: Boolean) :
    TimerState()
